...
{
//SFF_BEGIN
	const float ImwContainer::c_fTabHeight = 22.f;
	...
	void ImwContainer::Paint(/* int iX, int iY, int iWidth, int iHeight */)
	{
		...
		else if (HasWindowTabbed())
		{
			...
			if (!bAlone)
			{
				...
				ImGui::InvisibleButton("TabListButton", ImVec2(20, 20));
				ImGui::SameLine();
				...
				pDrawList->AddTriangleFilled(
					ImVec2(oButtonMin.x+6, oButtonMin.y+6),
					ImVec2(oButtonMax.x-6, oButtonMin.y+6),
					ImVec2(oButtonMin.x + oButtonSize.x/2.f, oButtonMin.y + oButtonSize.y-6),
					oLinesColor
				);
			...
			if (pActiveWindow != NULL)
			{
				bool custom = pActiveWindow->IsCustomRendered();

				ImVec4 oWindowBg = oStyle.Colors[ImGuiCol_WindowBg];
				ImVec4 oChildBg = oStyle.Colors[ImGuiCol_ChildWindowBg];
				ImVec4 oBorderColor = oStyle.Colors[ImGuiCol_Border];
				ImVec4 oBorderShadowColor = oStyle.Colors[ImGuiCol_BorderShadow];
				
				if (custom) {
					oStyle.Colors[ImGuiCol_Border] = ImVec4(oChildBg.x, oChildBg.y, oChildBg.z, 1.f);
				...
				ImGui::BeginChild(pActiveWindow->GetId(), ImVec2(0.f, 0.f), custom ? true : !pActiveWindow->IsFillingSpace(), ImGuiWindowFlags_HorizontalScrollbar);

				oStyle.Colors[ImGuiCol_ChildWindowBg] = oChildBg;
	...
	void ImwContainer::DrawTab(const ImwChar* pText, bool bFocused, ImVec2 oPos, float fStartLinePos, float fEndLinePos, float fMaxSize, ImVec2* pSizeOut)
	{
		...
		switch (oConfig.m_eTabColorMode)
		{
			...
			case ImwWindowManager::E_TABCOLORMODE_BACKGROUND:
				oNormalTab = ImColor(
					oStyle.Colors[ImGuiCol_ChildWindowBg].x,
					oStyle.Colors[ImGuiCol_ChildWindowBg].y,
					oStyle.Colors[ImGuiCol_ChildWindowBg].z,
					oStyle.Colors[ImGuiCol_ChildWindowBg].w
				);
				oSelectedTab = oStyle.Colors[ImGuiCol_TextDisabled];
		...
		pDrawList->AddRectFilled(oRectMin, oRectMax, !bFocused ? oSelectedTab : oNormalTab);
